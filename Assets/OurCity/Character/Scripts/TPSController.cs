...
namespace OurCity
{
    ...
    public class TPSController : MonoBehaviour
    {
		...
		void Move()
		{
			float targetSpeed = InputHandler.Instance.Sprint ? m_SprintSpeed : m_MoveSpeed;
			...
			float inputMagnitude = InputHandler.Instance.Move.magnitude;//m_AssetsInputs.IsAnalogMovement ? m_AssetsInputs.Move.magnitude : 1f;
			...
			Vector3 inputDirection = new Vector3(InputHandler.Instance.Move.x, 0.0f, InputHandler.Instance.Move.y).normalized;
		...
		void JumpAndGravity()
		{
			if (m_Grounded)
			{
				...
				if (InputHandler.Instance.Jump && m_JumpTimeoutDelta <= 0.0f)
				{
			...
			else
			{
				...
				InputHandler.Instance.Jump = false;
			}
