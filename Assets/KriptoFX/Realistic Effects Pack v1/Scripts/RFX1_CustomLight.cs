...
public class RFX1_CustomLight : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Shader.SetGlobalInt("RFX1_LightCount", lightCount);
        Shader.SetGlobalVectorArray("RFX1_LightPositions", ListToArrayWithMaxCount(lightPositions, MaxLightsCount));
        Shader.SetGlobalVectorArray("RFX1_LightColors", ListToArrayWithMaxCount(lightColors, MaxLightsCount));

        var ambientColor = SampleLightProbesUp(transform.position, 0.5f);
        Shader.SetGlobalColor("RFX1_AmbientColor", ambientColor);
    }

    void OnDisable()
    {
        Shader.SetGlobalInt("RFX1_LightCount", 0);
        Shader.SetGlobalVectorArray("RFX1_LightPositions", new[] { Vector4.zero });
        Shader.SetGlobalVectorArray("RFX1_LightColors", new[] { Vector4.zero });
