    ...
    public class MovingPlatformInspector : StateBehaviorInspector
    {
        ...
        protected override Action GetDrawCallback()
        {
            ...
            baseCallback += () =>
            {
                if (Foldout("Path")) {
                    ...
                    if (movementType.enumValueIndex == (int)MovingPlatform.PathMovementType.Random) {
                        EditorGUI.indentLevel++;
                        EditorGUILayout.PropertyField(PropertyFromName("m_RandomSeed"));
                        EditorGUI.indentLevel--;
                    }
                    EditorGUI.indentLevel--;
                }
        ...
        static void DrawMovingPlatformGizmo(MovingPlatform movingPlatform, GizmoType gizmoType)
        {
            ...
            for (int i = 0; i < movingPlatform.Waypoints.Length; ++i) {
                ...
                if (movingPlatform.MovementType == MovingPlatform.PathMovementType.Random) {
                    Gizmos.color = InspectorUtility.GetContrastColor(movingPlatform.GizmoColor);
                    Gizmos.DrawLine(movingPlatform.transform.position, movingPlatform.Waypoints[i].Transform.position);
                    if (movingPlatform.DrawDebugLabels) {
                        // Draw a distance in the center of the line.
                        var distance = decimal.Round((decimal)Vector3.Distance(movingPlatform.transform.position, movingPlatform.Waypoints[i].Transform.position), 3);
                        Handles.Label((movingPlatform.transform.position + movingPlatform.Waypoints[i].Transform.position) / 2, distance.ToString(CultureInfo.InvariantCulture), s_DebugLabelStyle);
                    }
                } else if (i > 0 && movingPlatform.Waypoints[i - 1].Transform != null && movingPlatform.MovementType != MovingPlatform.PathMovementType.Target) {
                    Gizmos.color = InspectorUtility.GetContrastColor(movingPlatform.GizmoColor);
