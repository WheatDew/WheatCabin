...
namespace Opsive.UltimateCharacterController.Objects.CharacterAssist
{
    ...
    public class MoveTowardsLocation : MonoBehaviour
    {
        ...
        {
            var forwardDirection = TargetRotation * Vector3.forward;
            if (m_RequireGrounded) {
                forwardDirection = Vector3.ProjectOnPlane(forwardDirection, rotation * Vector3.up);
            }
            return Vector3.Angle(forwardDirection, rotation * Vector3.forward) <= m_Angle / 2 + 0.001f;
        }
    }
}