        ...
        public override float GetDeltaYawRotation(float characterHorizontalMovement, float characterForwardMovement, float cameraHorizontalMovement, float cameraVerticalMovement)
        {
            ...
            if (m_LookInMoveDirection) {
                if (characterHorizontalMovement != 0 || (m_AllowDepthMovement ? characterForwardMovement : 0) != 0) {
                    ...
                    if (m_Path != null) {
                        lookRotation = Quaternion.LookRotation(Quaternion.LookRotation(Vector3.Cross(m_Path.GetTangent(m_Rigidbody.position, ref m_PathIndex), m_CharacterLocomotion.Up)) * inputVector.normalized);
                    } else {
                ...
                if (m_PlayerInput.IsCursorVisible()) {
                    var characterCenter = m_Rigidbody.position + (m_CharacterLocomotion.Up * m_CharacterLocomotion.Height / 2);
                    Vector3 forward;
                    if (m_Path != null) {
                        forward = Vector3.Cross(m_Path.GetTangent(m_Rigidbody.position, ref m_PathIndex), m_CharacterLocomotion.Up);
                    } else {
                        forward = m_LookSource.Transform.forward;
                    }
                    var localLookDirection = m_Rigidbody.InverseTransformDirection(forward);
                    // The vertical look direction can be ignored.
                    localLookDirection.y = 0;
                    m_HitPlane.SetNormalAndPosition(m_Rigidbody.TransformDirection(localLookDirection), characterCenter);
                    ...
                    if (m_HitPlane.Raycast(ray, out distance)) {
                        ...
                        var localHitPoint = m_Rigidbody.InverseTransformPoint(ray.GetPoint(distance));
                        localHitPoint.y = 0;
                        if (localHitPoint.magnitude > (m_LookRotateBuffer * Mathf.Max(1, m_CharacterLocomotion.Velocity.magnitude)) || m_InitialOrientation) {
                            ...
                            var rotation = Quaternion.LookRotation((m_Rigidbody.TransformPoint(localHitPoint) - characterCenter).normalized, m_CharacterLocomotion.Up);
